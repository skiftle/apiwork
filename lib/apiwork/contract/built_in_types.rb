...
module Apiwork
  module Contract
    # BuiltInTypes: Register Apiwork's built-in global descriptors
    #
    ...
    module BuiltInTypes
      class << self
        def reset!
          @registered_filter_types = Set.new
        ...
        def ensure_filter_descriptors_registered(schema_class)
          needed = determine_needed_filter_types(schema_class)
        ...
        def register_filter_descriptor(type_name)
          @registered_filter_types ||= Set.new
          return if @registered_filter_types.include?(type_name)

          @registered_filter_types.add(type_name)
        ...
        # Determine which filter types are needed based on schema's filterable attributes
        def determine_needed_filter_types(schema_class)
          types = Set.new
