...
An object defines a named shape:
...
The `:address` type is registered as `:invoice_address` in the API's type system. Within the contract, it is referenced by its short name.
...
An enum restricts a param to a fixed set of values:
...
Named enums are preferable when the same set of values is used in multiple places or when they should appear as distinct types in exports.

## Fragments

A fragment defines a reusable group of params that can be merged into objects. Unlike objects, fragments are invisible in exports — they exist purely for composition:
...
The params from `:timestamps` are inlined into the `:invoice` object. No separate type appears in OpenAPI, TypeScript, or Zod exports.
...
Types defined inside a contract are prefixed with the contract's identifier. The identifier is inferred from the class name:
...
The identifier can be overridden explicitly:
