...
`Boolean` — true if this is a boolean param
...
`Array<Boolean>`, `Symbol`, `nil` — enum values (Array) or reference name (Symbol)
...
`Boolean` — false — booleans do not support format constraints
