...
The built-in adapter maps Rails validation errors to [domain issues](../core/errors/domain-issues.md). The `detail` field in each issue is translatable.

Lookup order:

1. **API-specific**: `apiwork.apis.<path>.adapters.standard.domain_issues.<code>.detail`
2. **Global**: `apiwork.adapters.standard.domain_issues.<code>.detail`
...
See [Domain Issues](../core/errors/domain-issues.md) for the complete list of codes.
...
Use inline for single-language APIs. Use locale files when you need multiple languages or localized OpenAPI specs.
...
Apiwork provides English translations for all built-in error codes. Everything else — types, attributes, associations — needs explicit descriptions via inline options, locale files, or [Type Merging](../core/type-system/type-merging.md).
