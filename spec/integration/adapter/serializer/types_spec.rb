...
RSpec.describe 'Serializer types', type: :integration do
  ...
  describe 'error types' do
    it 'has error object with issues and layer' do
    ...
    it 'has layer enum with values' do
      expect(enums[:layer].values).to eq(%w[http contract domain])
    end

    it 'has issue object with expected fields' do
