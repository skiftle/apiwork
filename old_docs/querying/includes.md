...
1. **`include: :always` associations** - Always included in responses
2. **Filter/sort params** - Associations used in queries
3. **Explicit `include` params** - Manual control when needed

Mark associations as `include: :always` to include them in responses, and Apiwork handles the rest.
...
### 1. `include: :always` associations (always)
...
```ruby
class PostSchema < Apiwork::Schema::Base
  has_many :tags, schema: TagSchema, filterable: true  # include: :optional
end
```
...
# → No N+1 even though include: :optional!
```
...
# → Include author even if include: :optional
```
...
# Schema
class PostSchema < Apiwork::Schema::Base
  has_many :comments, include: :always   # Always in response
...
# Only comments in response (include: :always)
```

**Example 2: Filter on non-included association**
```bash
GET /posts?filter[tags][name]=ruby
# Auto eager-loads: { comments: {}, tags: {} }
# Comments in response (include: :always)
...
**Example 4: include: :always cannot be disabled**
```bash
GET /posts?include[comments]=false
# Contract validation error - :always associations cannot be excluded
# :always means truly always - no opt-out possible
```

## Controlling association inclusion

`include: :always` means "this association is part of the default response":
...
Nested `include: :always` associations are **automatically** eager-loaded:
...
class CommentSchema < Apiwork::Schema::Base
  belongs_to :user, schema: UserSchema, include: :always
end
```
...
# → Comments AND users both auto-included (nested :always)
```
...
**Note**: Associations with `include: :always` are **automatically** included at all nesting levels without needing explicit include params. Associations with `include: :optional` (the default) can still be included but require explicit include params.
...
```ruby
class PostSchema < Apiwork::Schema::Base
  has_one :featured_image, schema: ImageSchema, include: :always
end
```
...
### Skip `include: :always` associations

```ruby
class PostSchema < Apiwork::Schema::Base
  has_many :comments, include: :always  # Auto-included by default
end
```

```bash
GET /posts?include[comments]=false
# → Skips comments (even though include: :always)
...
# → Useful when tags aren't included by default anyway
```
...
- `include: :always` → Always eager-loaded and in response
- `include: :optional` + used in filter/sort → Eager-loaded but NOT in response
- `include: :optional` + not used → Not eager-loaded
...
class CommentSchema < Apiwork::Schema::Base
  belongs_to :post, schema: PostSchema, include: :always
end
```
...
Includes respect your schema's `include` flag:
...
class PostSchema < Apiwork::Schema::Base
  ...
  has_many :edit_logs, schema: EditLogSchema, include: :optional
end
```
...
# Ignored - edit_logs not defined in schema
...
### 1. `include: :always` → Always eager-loaded
```ruby
has_many :comments, include: :always
...
1. **Mark response associations as `include: :always`** - They'll auto eager-load
