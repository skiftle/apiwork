...
The `create` action expects a request body with `title` and `body`, both strings. Invalid requests are rejected before the controller runs.
...
In this mode, the contract is driven by its representation through an [adapter](../adapters/). Request shapes, response shapes, filter types, and sort options are derived automatically. All generated behavior remains fully customizable.
...
Abstract contracts serve as base classes for shared [imports](./imports.md) and configuration. Subclasses automatically become non-abstract.
