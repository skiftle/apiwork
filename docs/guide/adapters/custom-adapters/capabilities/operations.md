...
Operations run at runtime to process data. They filter, sort, paginate, or otherwise transform the data before serialization. Operations inherit from [`Adapter::Capability::Operation::Base`](/reference/adapter/capability/operation/base).
...
Without `target`, the operation runs for both collection and member responses.
...
Inside the block, type DSL methods (`integer`, `string`, `object`, `reference`, etc.) to define the shape. Access `options` to read capability configuration.
...
result(
  ...
  includes: [:customer, :lines],    # Associations to eager load
