...
Metadata options document attributes for API exports and client generation. These options affect generated OpenAPI, TypeScript, and Zod output but have no effect on runtime behavior.
...
| Format | Type | OpenAPI | Zod |
|--------|------|---------|-----|
| `:int32` | `integer` | `format: int32` | `z.number().int()` |
| `:int64` | `integer` | `format: int64` | `z.number().int()` |
| `:float` | `decimal` / `number` | `format: float` | `z.number()` |
| `:double` | `decimal` / `number` | `format: double` | `z.number()` |
