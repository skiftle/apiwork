...
Apiwork uses Rails' I18n. Error messages, action descriptions, and documentation strings can all be translated.

Translations resolve at runtime based on `I18n.locale`. Same API, different languages.

## Error Codes

When you call `respond_with_error :not_found`, Apiwork looks up the message:
...
All descriptions follow the same pattern:
...
Same structure, different locale files:
