...
On top of routing, each resource is linked to a [contract](../contracts/) and included in [exports](./exports.md).

## Path and Namespace

The path passed to `define` determines two things:

1. **Mount point** — where routes live (`/api/v1/posts`)
...
```ruby
resources :invoices, path: 'billing'
```
...
```ruby
resources :invoices, constraints: { id: /\d+/ }
```

Requests where `:id` is not numeric will not match. Works the same as Rails route constraints.
...
`reports` and `analytics` are read-only. `invoices` gets all five CRUD actions.
