...
## What Triggers Domain Issues

Domain issues are raised when business logic rejects valid input. How this happens depends on your adapter:
...
## Raising Domain Issues

How domain issues are raised depends on your adapter. The standard adapter uses ActiveRecord's error system:
...
def transfer
  ...
  expose account  # Adapter converts errors to domain issues
end
```
