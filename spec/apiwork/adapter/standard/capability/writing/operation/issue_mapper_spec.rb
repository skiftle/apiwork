...
RSpec.describe Apiwork::Adapter::Standard::Capability::Writing::Operation::IssueMapper do
  ...
  describe '.map' do
    ...
    context 'when record has base error' do
      let(:record) do
        build_record(errors: [IssueMapperMockError.new(attribute: :base, type: :insufficient_funds)])
      end

      it 'preserves error type as code' do
        expect(issues.first.code).to eq(:insufficient_funds)
      end
    ...
    context 'when record has base error with string message' do
      let(:record) do
        build_record(errors: [IssueMapperMockError.new(attribute: :base, type: 'Something went wrong')])
      end

      it 'falls back to :invalid' do
    ...
    context 'when record has attribute error with string message' do
      let(:record) do
        build_record(errors: [IssueMapperMockError.new(attribute: :number, type: 'must be special')])
      end
    ...
    context 'when record has has_many association with errors' do
      ...
      let(:record) do
        build_record(
          ...
          errors: [IssueMapperMockError.new(attribute: :base, type: :insufficient_funds)],
        )
      end
